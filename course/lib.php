...
function course_create_section($courseorid, $position = 0, $skipcheck = false) {
    ...
    if (method_exists($courseformat, 'set_default_options')) {
        $courseformat->set_default_options($cw);
    }
...
function course_get_cm_edit_actions(cm_info $mod, $indent = -1, $sr = null) {
    ...
    if (!isset($str)) {
        $str = get_strings(array(
            'delete', 'inactivezoom', 'activezoom', 'move', 'moveright', 'moveleft',
        ...
        $str->inactivezoom  = get_string('inactivezoom', 'local_brightcove');
        $str->activezoom  = get_string('activezoom', 'local_brightcove');
    }
...
// changes for zoom activity hidden dhasharath.k.
$defaulticonclass = 'iconsmall';

// First condition

if (has_capability('moodle/course:activityvisibility', $modcontext) && ($mod->modname !== 'zoom')) {
    ...
    $defaultAction = new action_menu_link_secondary(
        new moodle_url($baseurl, array('hide' => $mod->id)),
        new pix_icon('t/hide', '', 'moodle', array('class' => $defaulticonclass)),
        $str->modhide,
        [
            'class' => 'editing_hide',
    ...
    if ($stealth) {
        ...
        $actions[$action] = new action_menu_link_secondary(
            ...
            new pix_icon('t/block', '', 'moodle', array('class' => $defaulticonclass)),
            $str->makeavailable,
            [
...
// Second condition
if (($mod->modname == 'zoom') ) {
    $zoomstatus = $DB->get_field('local_zoom_status', 'id', ['cmid' => $mod->id, 'zoomid' => $mod->instance]);
    if(!$zoomstatus) {
    ...
    $zoomactivity = $mod->modname;

    $displayedoncoursepage = $mod->visible && $mod->visibleoncoursepage && $sectionvisible && $zoomactivity;
    $unavailable = !$mod->visible;
    $stealth = $mod->visible && (!$mod->visibleoncoursepage || !$sectionvisible);

    if ($displayedoncoursepage) {
        $actions['hide_zoom'] = new action_menu_link_secondary(
            new moodle_url($baseurl, array('hide_zoom' => $mod->id)),
            new pix_icon('t/hide', '', 'moodle', array('class' => 'iconsmall')),
            $str->inactivezoom,
            [
                'class' => 'editing_inactivezoom',
                'data-action' => ($usecomponents) ? 'cmZoomHide' : 'hide',
                'data-id' => $mod->id,
            ]
        );
    } else if (!$displayedoncoursepage && $sectionvisible) {
        $actions['show_zoom'] = new action_menu_link_secondary(
            new moodle_url($baseurl, array('show_zoom' => $mod->id)),
            new pix_icon('t/show', '', 'moodle', array('class' => 'iconsmall')),
            $str->activezoom,
            [
                'class' => 'editing_activezoom',
                'data-action' => ($usecomponents) ? 'cmZoomShow' : 'show',
                'data-id' => $mod->id,
            ]
        );
    }
