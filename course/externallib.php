    ...
    public static function get_course_contents($courseid, $options = array()) {
        ...
        if ($canupdatecourse or $course->visible
            ...
            foreach ($sections as $key => $section) {
                ...
                $sectionvalues['parent'] = $section->parent;
                
                // custom code added to get 'progress' added by dhasharath.k
                // start here.
                if($sectionvalues['id']) {
                    $coursemodule = $DB->count_records('course_modules', ['section' => $sectionvalues['id'], 'visible' => 1, 'completion' => 1]);
                    $compsql = "SELECT DISTINCT(cmc.id) as cmcid, cmc.userid  
                                FROM {course_modules} cm 
                                JOIN {course_modules_completion} cmc ON cmc.coursemoduleid = cm.module 
                                WHERE cm.section = :section AND cm.visible = :visible AND cm.completion = :completion AND cmc.userid = :userid AND cmc.completionstate = :completionstate";
                    $coursemodulecompletion = $DB->get_records_sql($compsql, ['section' => $sectionvalues['id'], 'visible' => 1, 'completion' => 1, 'userid' => $USER->id, 'completionstate' => 1]);
                  
                 
                   if ($coursemodule == 0) {
                        $progress = 0;
                    } else {
                        $progress = count($coursemodulecompletion) / $coursemodule * 100;
                    }

                    $sectionvalues['progress'] = round($progress);

                    // Custom changes added for the icon in the service.
                    $icon = $DB->get_field('course_format_options', 'value', ['sectionid' => $section->id, 'name' => 'icon', 'format' => 'flexsections']);

                    $iconurlslist = [
                        'Test' => $CFG->wwwroot.'/theme/horizon/pix/sectionicons/test.svg',
                        'Live Class' => $CFG->wwwroot.'/theme/horizon/pix/sectionicons/liveclass.svg',
                        'Practice Questions' => $CFG->wwwroot.'/theme/horizon/pix/sectionicons/practicequestions.svg',
                        'Study Material' => $CFG->wwwroot.'/theme/horizon/pix/sectionicons/studymaterial.svg',
                        'Reading' => $CFG->wwwroot.'/theme/horizon/pix/sectionicons/reading.svg',
                        'Video' => $CFG->wwwroot.'/theme/horizon/pix/sectionicons/video.svg',
                        'Doubt Solving' => $CFG->wwwroot.'/theme/horizon/pix/sectionicons/doubtsolving.svg',
                    ];

                    if (isset($iconurlslist[$icon])) {
                        $sectionvalues['icon'] = $iconurlslist[$icon];
                    } else {
                        $sectionvalues['icon'] = '';
                    }
                }
                ...
                if (empty($filters['excludemodules']) and !empty($modinfosections[$section->section])) {
                    foreach ($modinfosections[$section->section] as $cmid) {
                        ...
                        // custom code added for 'type' added by dhasharath.k
                        // start here.
                        if (($module['modname'] == 'quiz')) {
                            $mdtype = $DB->get_field('quiz', 'testtype', ['id' => $module['instance']]);
                            if($mdtype) {
                                $module['type'] = $mdtype;
                            } else {
                               $module['type'] = 0; 
                            ...
                            $mdtype = $DB->get_field('page', 'pagetype', ['id' => $module['instance']]);
                              if($mdtype) {
                                $module['type'] = $mdtype;
                            } else {
                               $module['type'] = 0; 
        ...
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    ...
                    'parent' => new external_value(PARAM_INT, 'Section Parent value.', VALUE_OPTIONAL),
                    'progress' => new external_value(PARAM_INT, 'progress value.', VALUE_OPTIONAL),
                    'icon' => new external_value(PARAM_RAW, 'Section icon.', VALUE_OPTIONAL),
