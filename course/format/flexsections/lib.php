...
class format_flexsections extends core_courseformat\base {
    ...
    /**
     * Returns true on setting up the defaults for the section.
     *
     * @return bool
     */
    public function set_default_options($section) {
        global $DB;
        $defaults = ['parent' => 0, 'collapsed' => 1];
        foreach($defaults AS $key => $value) {
            $record_exists = $DB->record_exists('course_format_options', ['name' => $key, 'format' => 'flexsections', 'sectionid' => $section->id, 'courseid' => $section->course]);
            if (!$record_exists) {
                $dataobject = (object)['name' => $key, 'format' => 'flexsections', 'sectionid' => $section->id, 'courseid' => $section->course, 'value' => $value];
                $DB->insert_record('course_format_options', $dataobject);
            }
        }
    }
    /**
    ...
    /**
     * Returns the display icon of the given section that the course prefers.
     *
     * @param int|stdClass $section Section object from database or just field course_sections.section
     * @return string Display icon that the course format prefers, e.g. "Topic 2"
     */
    public function get_section_icon($section) {
        global $DB, $OUTPUT, $CFG;
        if (!is_object($section)) {
            $section = $this->get_section($section);
        }
        $icon = $DB->get_field('course_format_options', 'value', ['sectionid' => $section->id, 'name' => 'icon', 'format' => 'flexsections']);
        // $OUTPUT->pix_icon('sectionicons/test', get_string('test', 'format_flexsections'), 'theme_horizon')
        $iconslist = ['Test' =>  $OUTPUT->pix_icon('sectionicons/liveclass', get_string('test', 'format_flexsections'), 'theme_horizon'), 
            'Live Class' => file_get_contents($CFG->dirroot.'/theme/horizon/pix/sectionicons/liveclass.svg'),
            'Practice Questions' => file_get_contents($CFG->dirroot.'/theme/horizon/pix/sectionicons/practicequestions.svg'),
            'Study Material' => file_get_contents($CFG->dirroot.'/theme/horizon/pix/sectionicons/studymaterial.svg'),
            'Reading' => file_get_contents($CFG->dirroot.'/theme/horizon/pix/sectionicons/reading.svg'),
            'Video' => file_get_contents($CFG->dirroot.'/theme/horizon/pix/sectionicons/video.svg'),
            'Doubt Solving' => file_get_contents($CFG->dirroot.'/theme/horizon/pix/sectionicons/doubtsolving.svg'),
        ];
        if (isset($iconslist[$icon])){
    ...
    public function get_section_performance($courseid, $sectionid, &$sectioninfo){ 
        global $DB, $USER; 
        // return 0;
        $sectondata = $DB->get_record_sql("SELECT COUNT(cm.id) as sectionmodulecount, count(cmc.id) as completedmodulecount FROM {course_modules} cm LEFT JOIN {course_modules_completion} cmc ON cmc.coursemoduleid = cm.id AND cmc.userid = :userid AND cmc.completionstate > 0 WHERE cm.completion = 1 AND cm.section = :sectionid AND cm.course = :courseid ", ['sectionid' => $sectionid, 'courseid' => $courseid, 'userid' => $USER->id]);
        if (isset($sectioninfo['sectionmodulecount'])) {
            $sectioninfo['sectionmodulecount'] += $sectondata->sectionmodulecount;
        } else {
            $sectioninfo['sectionmodulecount'] = $sectondata->sectionmodulecount;
        }
        if (isset($sectioninfo['completedmodulecount'])) {
            $sectioninfo['completedmodulecount'] += $sectondata->completedmodulecount;
        } else {
            $sectioninfo['completedmodulecount'] = $sectondata->completedmodulecount;
        }

         $childsections = $DB->get_records_sql("SELECT cfo.id, cfo.sectionid FROM {course_format_options} cfo JOIN {course_sections} cs ON cs.section = cfo.value AND cs.course = cfo.courseid WHERE cs.id = :sectionid AND cfo.format like 'flexsections' AND cfo.name like 'parent' AND cfo.courseid = :courseid AND cfo.value <> 0 ", ['sectionid' => $sectionid,  'courseid' => $courseid]);
        if (!empty($childsections)) {
            foreach ($childsections AS $csection) {
                $this->get_section_performance($courseid, $csection->sectionid, $sectioninfo);
            }
        }
        if (!empty($sectioninfo) && !empty($sectioninfo['completedmodulecount']) && $sectioninfo['sectionmodulecount'] > 0 ) {
            return ($sectioninfo['completedmodulecount']/$sectioninfo['sectionmodulecount'])*100;
        } else {
            return 0;
        } 
    }
    ...
    public function section_format_options($foreditform = false): array {
        return array(
            ...
            'icon' => array(
                'type' => PARAM_RAW,
                'label' => get_string('selecticon', 'format_flexsections'),
                'element_type' => 'autocomplete',
                'element_attributes' => array(
                    array(
                        null => new lang_string('selecticon', 'format_flexsections'),
                        'Test' => new lang_string('test', 'format_flexsections'),
                        'Live Class' => new lang_string('liveclass', 'format_flexsections'),
                        'Practice Questions' => new lang_string('practicequestions', 'format_flexsections'),
                        'Study Material' => new lang_string('studymaterial', 'format_flexsections'),
                        'Reading' => new lang_string('reading', 'format_flexsections'),
                        'Video' => new lang_string('video', 'format_flexsections'),
                        'Doubt Solving' => new lang_string('doubtsolving', 'format_flexsections'),
                    )
                ),
                'default' => 0,
                'cache' => true,
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) {
        ...
        if($forsection){
            ...
            // Section Parent Drop Down    
            $issubsectionsql = "SELECT value FROM {course_format_options}
            WHERE courseid = :courseid AND name = 'parent' AND sectionid =:sectionid";
            $parentid = $DB->get_field_sql($issubsectionsql, ['courseid' => $COURSE->id, 'sectionid' => $id]);
            $parentsectionid = $DB->get_field('course_sections', 'id',['course' => $COURSE->id, 'section' => $parentid]);
            $sql = "SELECT cs.section , CASE WHEN cs.name IS NOT NULL THEN cs.name else concat('Topic', ' ', cs.section) end as sectionname FROM {course_sections} cs
            WHERE cs.course = :courseid AND cs.section > 0 AND cs.id <> :id ORDER BY cs.section ";
            $sections = $DB->get_records_sql_menu($sql, ['courseid' => $COURSE->id, 'id' => $id]);
            $element1 = $mform->addElement('autocomplete', 'sectionparent', get_string('sectionparent', 'format_flexsections'), [null =>get_string('selectparent', 'format_flexsections')]+$sections);
            $mform->setDefault('sectionparent', $parentid);
            array_unshift($elements, $element1);
        }
        return $elements;
    }
    ...
    public function update_section_format_options($data) {
        $data = (array)$data;
        if (isset($data['sectionparent'])) {
            $data['parent'] = $data['sectionparent'];
        }
