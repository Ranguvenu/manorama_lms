...
class cm implements named_templatable, renderable {
    ...
    public function export_for_template(renderer_base $output): stdClass {
        ...
        // Added custom class in case of completed the activity.
        $completion = $DB->record_exists_sql("SELECT cmc.id FROM {course_modules_completion} cmc WHERE cmc.userid = :userid AND cmc.completionstate <> 0 AND cmc.coursemoduleid = :cmid ", array('cmid' => $this->mod->id, 'userid' => $USER->id));
        if ($completion) {
            $data->activitycompletionclass = 'activity_completed';
        }
        // Calculated fields.
        if (!empty($data->url)) {
            $data->hasurl = true;
    ...
    protected function add_completion_data(stdClass &$data, renderer_base $output): bool {
        ...
        if ($showcompletioninfo || !empty($activitydates)) {
            ...
            $activityinfodata->modtype = $this->mod->modname;
        }
    ...
    protected function add_format_data(stdClass &$data, array $haspartials, renderer_base $output): bool {
        ...
        if ((!$this->mod->visible) && ($this->mod->modname !== 'zoom')) {
            // This module is hidden but current user has capability to see it.
            $data->modhiddenfromstudents = true;
            $data->zoominactivefromstudents = false;
            $result = true;
        } else if (($this->mod->visible) && ($this->mod->modname == 'zoom')) {
            // This module is hidden but current user has capability to see it.
            $zoominactiveby = $DB->get_field('local_zoom_status', 'usercreated', ['cmid' => $this->mod->id, 'zoomid' => $this->mod->instance]);
            if($zoominactiveby) {
                if(has_capability('local/packages:accesssitefromlms', context_system::instance())) {
                    $data->modhiddenfromstudents = false;
                    $data->zoominactivefromstudents = true;
                    $data->username = $DB->get_field('user', 'username', ['id' => $zoominactiveby]);
                    $data->text = get_string('zoomerr_meetingnotfound_info', 'mod_zoom');
                    $data->isnotstudent = true;
                }else{

                    $data->modhiddenfromstudents = false;
                    $data->zoominactivefromstudents = true;
                    $data->isnotstudent = false;
                    $data->username = $DB->get_field('user', 'username', ['id' => $zoominactiveby]);
                    $data->text = get_string('zoomerr_meetingnotfound_info_to_user', 'mod_zoom');
