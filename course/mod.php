...
$showzoom         = optional_param('show_zoom', 0, PARAM_INT);
$hidezoom          = optional_param('hide_zoom', 0, PARAM_INT);

// This page should always redirect
$url = new moodle_url('/course/mod.php');
foreach (compact('indent','update','hide', 'hide_zoom', 'show', 'show_zoom', 'copy','moveto','movetosection','delete','course','cancelcopy','confirm') as $key=>$value) {
    if ($value !== 0) {
        $url->param($key, $value);
    }
}
...
if ((!empty($movetosection) or !empty($moveto)) and confirm_sesskey()) {
...
} else if ((!empty($hidezoom)) && (confirm_sesskey())) {
    $cm     = get_coursemodule_from_id('', $hidezoom, 0, true, MUST_EXIST);
    $course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
    ...
    // if (set_coursemodule_visible($cm->id, 0)) {
    if(!$DB->record_exists('local_zoom_status', ['cmid' => $cm->id, 'zoomid' => $cm->instance])){

        $data = new stdClass();
        $data->userid = $USER->id;
        $data->status = 0;
        $data->zoomid = $cm->instance;
        $data->cmid = $cm->id;
        $data->timecreated = time();
        $data->usercreated = $USER->id;

        $record = $DB->insert_record('local_zoom_status', $data);
        
        if($record) {
            $sendnotification = (new local_masterdata\api())->send_zoom_cancellation_notification($cm);
        }
    }
        
        \core\event\course_module_updated::create_from_cm($cm, $modcontext)->trigger();
        $zoom = $DB->get_record('zoom', ['id' => $cm->instance, 'course' => $cm->course]);
...
} else if (!empty($showzoom) and confirm_sesskey()) {
    list($course, $cm) = get_course_and_cm_from_cmid($showzoom);
    require_login($course, false, $cm);
    require_capability('moodle/course:activityvisibility', $cm->context);
    $section = $cm->get_section_info();
