...
class event_exporter_base extends exporter {
    ...
    public function __construct(event_interface $event, $related = []) {
        ...
        if ($cm = $event->get_course_module()) {
            ...
            $data->timeend =  0;
            if (component_callback_exists($component, 'core_calendar_get_event_endtime')) {
                $data->timeend = component_callback(
                    $component,
                    'core_calendar_get_event_endtime',
                    [$cm->get('id'), $cm->get('instance')]
                );
            }
    ...
    protected static function define_properties() {
        return [
            ...
            'timeend' => ['type' => PARAM_INT, 'optional' => true],
