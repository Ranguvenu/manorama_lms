...
class field_controller extends \core_customfield\field_controller {
    ...
    public function get_options(): array {
        global $DB;
            // $sectionslist = $DB->get_records_sql_menu("SELECT id,(CASE WHEN name IS NULL THEN CONCAT('Chapter',section) ELSE name END) as fullname from {course_sections} where 1=1");
        $sectionslist = $DB->get_records_sql_menu("SELECT lc.id AS id, lc.name AS fullname 
