...
class field_controller extends \core_customfield\field_controller {
    ...
    public static function get_options_array(\core_customfield\field_controller $field) : array {
        ...
        return $field->get_options_source();
    }
     ...
     * @return array
     */
    public function get_options_source(): array {
        global $DB;
        $sourcelist= $DB->get_records_sql_menu("SELECT id,name FROM {local_question_sources} WHERE 1=1 ORDER by id DESC");
        $sources = $sourcelist;
        return $sources;
    }
    ...
    public function course_grouping_format_values($values): array {
        $options = $this->get_options_source();
        $ret = [];
    ...
    public function parse_value(string $value) {
        return (int) array_search($value, $this->get_options_source());
    }
}
