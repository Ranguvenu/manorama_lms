...
if (empty($recordings)) {
    ...
    $PAGE->requires->js_call_amd('mod_zoom/iframepopup', 'init', []);
    foreach ($recordings as $grouping) {
        ...
        foreach ($grouping as $recording) {
            ...
            if ($iszoommanager || intval($recording->showrecording) === 1) {
                ...
                $params = ['id' => $zoom->id, 'recordingid' => $recording->id];
                $recordingurl = new moodle_url('/mod/zoom/loadrecording.php', $params);
                $linkattributes = [];
                $linkattributes['onclick'] = '(function(e){ require("mod_zoom/iframepopup").init({ id: '.$zoom->id.', recordingid:'.$recording->id.' }) })(event)';
                $linkattributes['title'] = $recordingurl;
                // $recordinglink = html_writer::link($recordingurl, $recording->name);
                $recordinglink = html_writer::link('javascript:void(0)', $recording->name, $linkattributes);
