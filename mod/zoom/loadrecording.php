...
// Changes for straming content from brightcove.
$brightcove_plugin_exist = \core_component::get_plugin_directory('local', 'brightcove');
if($brightcove_plugin_exist){

    $brightcoverecording = $DB->get_record('local_brightcove_recording', array('zoomrecordingid' => $rec->zoomrecordingid, 'meetinguuid' => $rec->meetinguuid));
    if($brightcoverecording){
        echo $OUTPUT->header();
            echo $OUTPUT->render_from_template('local_brightcove/embedd_video', ['settings' => get_config('local_brightcove'), 'recordingobj' => $brightcoverecording, 'userinfo' => $USER]);
        echo $OUTPUT->footer();
    }else{
        // $templatedata = [
        //     'url' => $rec->externalurl,
        // ];
        // echo $OUTPUT->render_from_template('mod_zoom/iframecontent', $templatedata);
        redirect($nexturl);
    }
}else{
    redirect($nexturl);
