...
function zoom_add_instance(stdClass $zoom, mod_zoom_mod_form $mform = null) {
    global $CFG, $DB;
    $modulecontext = context_module::instance($zoom->coursemodule);
    ...
    if (!empty($zoom->classnotes)) {
        file_save_draft_area_files($zoom->classnotes, $modulecontext->id, 'mod_zoom', 'classnotes', $zoom->classnotes);
    }
    if (!empty($zoom->lessonplan)) {
        file_save_draft_area_files($zoom->lessonplan, $modulecontext->id, 'mod_zoom', 'lessonplan', $zoom->lessonplan);
    }
    ...
    $zoomdate =  strtotime(date("d-m-Y",$zoom->start_time)) ;
    $starttime = strtotime(date('Y-m-d H:i', strtotime("today 7am")));
    $endtime = strtotime(date('Y-m-d H:i', strtotime("tomorrow 7am")));
    $currenthour =(int)date('H', time());
    if (($currenthour > 7 && $zoom->start_time > $starttime && $zoom->start_time <= $endtime) || ($todaydate == $zoomdate && ($currenthour < 7))){
        (new local_masterdata\api())->send_zoom_invite_notification($zoom,false);
    }
...
function zoom_update_instance(stdClass $zoom, mod_zoom_mod_form $mform = null) {
    global $CFG, $DB;
    require_once($CFG->dirroot . '/mod/zoom/locallib.php');
    $ctx = context_module::instance($zoom->coursemodule);
    ...
    if($zoom->instance) {
        $zoomexist = $DB->get_record('zoom', ['id' => $zoom->instance]);
        if ($zoomexist->start_time != $zoom->start_time) {
            (new local_masterdata\api())->existing_notification_status_update($zoom);
        
            $starttime = strtotime(date('Y-m-d H:i', strtotime("today 7am")));
            $endtime = strtotime(date('Y-m-d H:i', strtotime("tomorrow 7am")));          
            if ($zoomexist->start_time > $starttime && $zoomexist->start_time <= $endtime ){
                (new local_masterdata\api())->send_zoom_update_notification($zoom);
            }
        }
    }
    ...
    if (!empty($zoom->classnotes)) {
        file_save_draft_area_files($zoom->classnotes, $ctx->id, 'mod_zoom', 'classnotes', $zoom->classnotes);
    }
    if (!empty($zoom->lessonplan)) {
        file_save_draft_area_files($zoom->lessonplan, $ctx->id, 'mod_zoom', 'lessonplan', $zoom->lessonplan);
    }
...
function zoom_file_lessonplan_path($lessonplanid) {
    global $DB;
        $fileurl = false;
        $sql = "SELECT * FROM {files} WHERE itemid = :file_id AND component = 'mod_zoom' AND filearea = 'lessonplan' AND filename != '.' ORDER BY id DESC";
        $filedata = $DB->get_record_sql($sql, array('file_id' => $lessonplanid), 1);
    if (!empty($filedata)) {
        $fileurl = moodle_url::make_pluginfile_url($filedata->contextid, $filedata->component, $filedata->filearea, $filedata->itemid, $filedata->filepath, $filedata->filename);
        $fileurl = $fileurl->out();
    } 
    return $fileurl;
}
function zoom_file_classnotes_path($classnoteid) {
    global $DB;
        $fileurl = false;
        $sql = "SELECT * FROM {files} WHERE itemid = :file_id AND component = 'mod_zoom' AND filearea = 'classnotes' AND filename != '.' ORDER BY id DESC";
        $filedata = $DB->get_record_sql($sql, array('file_id' => $classnoteid), 1);
    if (!empty($filedata)) {
        $fileurl = moodle_url::make_pluginfile_url($filedata->contextid, $filedata->component, $filedata->filearea, $filedata->itemid, $filedata->filepath, $filedata->filename);
        $fileurl = $fileurl->out();
    } 
...
function zoom_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    ...
    $fs = get_file_storage();
    $file = $fs->get_file($context->id, 'mod_zoom', $filearea, $itemid, $filepath, $filename);
    if (!$file) {
        return false;
    }
    send_file($file, $filename, null, 0, false, 0, '', false, $options);
}
function zoom_get_viewcard_data($zoom) {
    $data = [];
    // $data['zoommeetingdate'] = userdate($zoom->start_time, get_string('strfshttimedaydate', 'mod_zoom'));
    // $starttime = userdate($zoom->start_time, get_string('strftimetime12', 'langconfig'));
    // $endtime = userdate($zoom->start_time + $zoom->duration, get_string('strftimetime12', 'langconfig'));
    // $data['zoommeetingtime'] = $starttime . ' - ' . $endtime;
    $data['timestart'] = $zoom->start_time;
    $data['timeend'] = $zoom->start_time + $zoom->duration;

    $data['title'] = $zoom->name;
    $data['summary'] = $zoom->intro;
    $data['lessonplan'] = zoom_file_lessonplan_path($zoom->lessonplan);
    $data['classnotes'] = zoom_file_classnotes_path($zoom->classnotes);
    return $data;
}
function mod_zoom_coursemodule_standard_elements($formwrapper, $mform) {
    global $CFG, $COURSE;
    if($formwrapper->get_current()->modulename == 'quiz'){
        $mform->addElement('header', 'questiontype', get_string('questiontype', 'quiz'));
        $radioarray = array();
        $radioarray[] = $mform->createElement('radio', 'testtype', '', get_string('test', 'quiz'), 0/* , $attributes */);
        $radioarray[] = $mform->createElement('radio', 'testtype', '', get_string('practicetest', 'quiz'), 1);
        $mform->addGroup($radioarray, 'radioar', '', array(' '), false);
    } else if ($formwrapper->get_current()->modulename == 'page') {
        $mform->addElement('header', 'pgtype', get_string('pagetype', 'page'));
        $radioarray = array();
        $radioarray[] = $mform->createElement('radio', 'pagetype', '', get_string('learningcontent', 'page'), 0);
        $radioarray[] = $mform->createElement('radio', 'pagetype', '', get_string('video', 'page'), 1);
        $mform->addGroup($radioarray, 'radioar', '', array(' '), false);
    }    
}
function mod_zoom_output_fragment_iframepopup($args) {
    global $DB, $OUTPUT;
    // $params = ['id' => $args['id'], 'recordingid' => $args['recordingid']];
    ...
    if (!empty($rec)) {
        $recordingurl = (new moodle_url($rec->externalurl))->out();
    }
    $templatedata = [
        'url' => $recordingurl,
    ];
    $output = $OUTPUT->render_from_template('mod_zoom/iframecontent', $templatedata);
    return $output;
}
