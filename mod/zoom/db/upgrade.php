...
function xmldb_zoom_upgrade($oldversion) {
    ...
    if ($oldversion < 2023082400.01) {
        $table = new xmldb_table('zoom');

        // Define and conditionally add field registration.
        $field = new xmldb_field('classnotes', XMLDB_TYPE_INTEGER, '20', null, null, null, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        // Define and conditionally add field registration.
        $field1 = new xmldb_field('lessonplan', XMLDB_TYPE_INTEGER, '20', null, null, null, null);
        if (!$dbman->field_exists($table, $field1)) {
            $dbman->add_field($table, $field1);
        }
        // Zoom savepoint reached.
        upgrade_mod_savepoint(true, 2023082400.01, 'zoom');
    }
    return true;
}
