...
function zoom_get_user_id($required = true) {
    ...
    $adminuseremail=get_config('zoom', 'adminuseremail');
    if(is_siteadmin() || has_capability('local/brighcove:managezoomactivity',$context)) {
        $user = $DB->get_record('user', ['email' => $adminuseremail]);
    }else{
        $user = $USER;
    }
...
function zoom_load_meeting($id, $context, $usestarturl = true) {
    ...
    // if(is_siteadmin() || has_capability('local/brighcove:managezoomactivity',$systemcontext)) {
