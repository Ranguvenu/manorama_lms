...
class mod_zoom_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        // Adding the "file" fieldset, where all two file will picked.
        $mform->addElement('header', 'general', get_string('filesforzoom', 'zoom'));
        $mform->addElement('filemanager', 'classnotes', get_string('classnotes', 'zoom'), null, array('accepted_types' => '*'));
        $mform->addElement('filemanager', 'lessonplan', get_string('lessonplan', 'zoom'), null, array('accepted_types' => '*'));
        ...
        // Add duration.
        $mform->addElement('duration', 'duration', get_string('zduration', 'zoom'), ['optional' => false]);
    ...
    public function data_preprocessing(&$defaultvalues) {
        ...
        if ($this->current && $this->current->coursemodule) {
            $cm = get_coursemodule_from_instance('zoom', $this->current->id, 0, false, MUST_EXIST);
        ...
        $draftitemid = file_get_submitted_draft_itemid('lessonplan');
        file_prepare_draft_area($draftitemid, $ctx->id, 'mod_zoom', 'lessonplan',$this->current->lessonplan, array('subdirs' => true), $this->current->lessonplan);

        $defaultvalues['lessonplan'] = $draftitemid;

        $draftitemid1 = file_get_submitted_draft_itemid('classnotes');
        // print_object($this->current);exit;
        file_prepare_draft_area($draftitemid1, $ctx->id, 'mod_zoom', 'classnotes', $this->current->classnotes, array('subdirs' => true), $this->current->classnotes);

        $defaultvalues['classnotes'] = $draftitemid1;
