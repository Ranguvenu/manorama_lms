...
$zoomcancel = $DB->get_field('local_zoom_status', 'id', ['cmid' => $PAGE->cm->id, 'zoomid' => $PAGE->cm->instance]);
if ($showrecreate) {
...
if ($zoom->show_schedule) {
    ...
    // Output table.
    $table->attributes['class'] = 'hidden'; // Hiding the table. 
    echo html_writer::table($table);
    $data = zoom_get_viewcard_data($zoom);
    if ($link == false && $zoom->start_time + $zoom->duration < time()) {
        $linkurl = new \moodle_url('/mod/zoom/recordings.php', ['id' => $cm->id]);
        $link = html_writer::link($linkurl, get_string('recordings', 'mod_zoom'), ['class' => 'btn btn-primary']);
    }
    $data['link'] = $link;
    $data['status'] = $status;
    $data['inprogress'] = $inprogress;
    echo $OUTPUT->render_from_template('mod_zoom/zoom_card', $data);
}
