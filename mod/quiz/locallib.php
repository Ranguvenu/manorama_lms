...
function quiz_start_new_attempt($quizobj, $quba, $attempt, $attemptnumber, $timenow,
    ...
    if ($randomfound) {
        ...
        $classname = "\\local_questions\\bank\\randomquestion_loader";
        if(class_exists($classname)){
        $randomloader = new \local_questions\bank\randomquestion_loader($qubaids, $usedquestionids);
        }else{
        ...
        foreach ($quizobj->get_questions() as $questiondata) {
            ...
            ///For Random Questions Based on custom fields Added Starts//
            $customfielddata=json_decode($questiondata->filtercondition);
            $goalvalue = array_keys((array) $customfielddata->goalid);
            $goalid = $goalvalue[0];

            $boardvalue = array_keys((array) $customfielddata->boardid);
            $boardid = $boardvalue[0];

            $classvalue = array_keys((array) $customfielddata->classid);
            $classid = $classvalue[0];
            
            $coursevalue = array_keys((array) $customfielddata->courseid);
            $subjectid = $coursevalue[0];
             
            $unitvalue = array_keys((array) $customfielddata->coursetopicid);
            $unitid = $unitvalue[0];

            $chaptervalue = array_keys((array) $customfielddata->chapterid);
            $chapterid = $chaptervalue[0];

            $topicvalue = array_keys((array) $customfielddata->unitid);
            $topicid = $topicvalue[0];

            $conceptvalue = array_keys((array) $customfielddata->conceptid);
            $conceptid = $conceptvalue[0];


            ///For Random Questions Based on custom fields Added Ends//
            ...
            $questionid = $randomloader->get_next_question_id($questiondata->category,
                    $questiondata->randomrecurse, $tagids, $goalid, $boardid, $classid,
