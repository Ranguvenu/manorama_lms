            ...
            foreach ($quizzes as $quiz) {
                ...
                if (has_capability('mod/quiz:view', $context)) {
                    ...
                    $sql = "SELECT count(id)
                              FROM {quiz_slots}
                             WHERE quizid = :quizid";
                    $questioncount = $DB->count_records_sql($sql, ['quizid' => $quiz->id]);
                    if ($questioncount > 0) {
                        $quizdetails['questionscount'] = $questioncount;
                    } else {
                        $quizdetails['questionscount'] = 0;
                    }

                    if ($quiz->timelimit > 0) {
                        $quizdetails['duration'] = format_time($quiz->timelimit);
                    } else {
                        $quizdetails['duration'] = 'Open';
                    }
                ...
                'quizzes' => new external_multiple_structure(
                    new external_single_structure(array_merge(
                        ...
                        [
                            ...
                            'questionscount' => new external_value(PARAM_INT, 'Count of the questions of quiz', VALUE_OPTIONAL),
                            'duration' => new external_value(PARAM_RAW, 'Duration of quiz', VALUE_OPTIONAL),
                        ]
                    ))
                ),
    ...
    public static function process_attempt($attemptid, $data, $finishattempt = false, $timeup = false, $preflightdata = []) {
        ...
        if (!empty($data)) {
            ...
            foreach ($data as $k => $value) {
                $datanamevalue = explode(':', $data[$i]['name']);
                if (!empty($datanamevalue)) {
                    $slotsarray = explode('_', $datanamevalue[1]);
                    $slot = $slotsarray[0];
                    $params = [];
                    $params['questionusageid'] = $uniqueid;
                    $params['slot'] = $slot;
                    $questionid = $DB->get_field('question_attempts', 'questionid', $params);
                    $sql = "UPDATE {question_attempts} set flagged = 1
                             WHERE questionid = :questionid
                               AND questionusageid = :quid";
                    $updateflag = $DB->execute($sql, ['questionid' => $questionid, 'quid' => $uniqueid]);
                }
                $i ++;
            }
        }
