...
class renderer extends plugin_renderer_base {
    ...
    public function review_page(quiz_attempt $attemptobj, $slots, $page, $showall,
        ...
        $output = '';
        $output .= $this->header();
        // passing attemptobj to get user grades.
        $output .= $this->review_summary_table($summarydata, $page, $attemptobj);
    ...
    public function navigation_panel(navigation_panel_base $panel) {
        ...
        $output .= $this->render_from_template('mod_quiz/quiz_navstatus',array());
    ...
    public function start_attempt_page(quiz_settings $quizobj, preflight_check_form $mform) {
        ...
        /**  MU-189 **/
        // Custom code added for rendering the content.
        $quiz = $quizobj->get_quiz();
        $data = quiz_get_attempt_preview_data($quiz);
        $data['attemptscount'] = count(quiz_get_user_attempts($quiz->id, $USER->id, 'finished', true));
        $attempturl = $quizobj->start_attempt_url();
        $attempturl->param('showintro', 0);
        $data['startattempturl'] = htmlspecialchars_decode($attempturl->out(true));
        $data['additional_summary'] = $mform->render();
        $output .= $this->render_from_template('mod_quiz/quiz_view', $data);
        $output .= $this->footer();
        return $output;
    }
    ...
    public function attempt_page($attemptobj, $page, $accessmanager, $messages, $slots, $id,
        ...
        $output .= $this->during_attempt_tertiary_nav(new moodle_url('/course/view.php', ['id' => $attemptobj->get_cm()->course]));
...
public function attempt_form($attemptobj, $page, $slots, $id, $nextpage) {
    ...
    // Get the unique ID of the current attempt.
    ...
    foreach ($slots as $slot) {
        ...
        // Get the question attempt record for the current slot.
        $qarecord = $DB->get_record('question_attempts', [
        ...
        if ($qarecord) {
            ...
            $dm->update_question_attempt_flag($uniqueid, $qarecord->questionid, $qarecord->id, $slot, $newstate);
        }
    }
    ...
    public function view_table_heading() {
        return $this->heading(get_string('summaryofattempts', 'quiz'), 5);
    }
