 ...
 */
//Updating the maximum attemts as 25 instead of 10.
define('QUIZ_MAX_ATTEMPT_OPTION', 25);
...
function mod_quiz_output_fragment_quiz_question_bank($args) {
    $classname = '\\local_questions\\local\\questionbank';
        if(class_exists($classname)){
            $classobj = new $classname();
            if(method_exists($classobj, 'override_quiz_fragment')){
                return $classobj->override_quiz_fragment($args);
            }
        }
    global $CFG, $DB, $PAGE;
    require_once($CFG->dirroot . '/mod/quiz/locallib.php');
...
function mod_quiz_output_fragment_add_random_question_form($args) {
  ...
  $classname = "\\local_questions\\form\\addrandom_form";
    if(class_exists($classname)){
        $form = new \local_questions\form\addrandom_form(
            new \moodle_url('/local/questions/addrandom.php'),
            $formoptions,
            'post',
            '',
            null,
...
/**
 * Callback to fetch the activity event endtime info.
 *
 * @param int $instance The event instance.
 * @return int the event endtime.
 */
function mod_quiz_core_calendar_get_event_endtime($instance) {
    global $DB;
    return (int) $DB->get_field('quiz', 'timeclose', array('id' => $instance));
}
...
function quiz_get_attempt_preview_data($quiz) {
    global $DB, $USER, $CFG;
    $modulecontext = context_module::instance($quiz->cmid);
    $data = [];
    $data['name'] = $quiz->name;
    $data['summary'] = $quiz->intro;
    $duration = 0;
    if ($quiz->timelimit != 0) {
        $duration = $quiz->timelimit;
        $data['timeend'] = true;
        $data['timeends'] = $quiz->timeopen + $quiz->timelimit;
    }
    //  else {
    //     $duration = $quiz->timeclose - $quiz->timeopen;
    // }
    if ($duration > 0) {
        $data['duration'] = format_time($duration);
    } else {
        $data['duration'] = 'Open';
    }
    if ($quiz->timeopen > 0) {
        $data['datestart'] = true;
        $data['datestarts'] = $quiz->timeopen;
    } else {
        $data['datestart'] = false;
        $data['datestarts'] = 'N/A';
    }
    if ($quiz->timeopen > 0) {
        $data['timestart'] = true;
        $data['timestarts'] = $quiz->timeopen;
    } else {
    ...
    if ($quiz->timeclose > 0) {
        $data['timeend'] = true;
        $data['timeends'] = $quiz->timeclose;
    } else {
        $data['timeend'] = false;
        $data['timeends'] = 'N/A';
    }
    $data['typeofevent'] = get_string('mcqtest', 'block_notification');
    $data['qcount'] = $DB->count_records('quiz_slots', ['quizid' => $quiz->id]);
    if (!empty($quiz->grade)) {
        // replaced some grade with maxgrade.
        // $data['qmarks'] = round($quiz->sumgrades);
        $data['qmarks'] = round($quiz->grade);
    } else {
        $data['qmarks'] = 0;
    }
    $sql = "SELECT gg.*
    ...
    $graderecord = $DB->get_record_sql($sql, ['userid' => $USER->id, 'iteminstance' => $quiz->id]);
    if (!empty($graderecord->finalgrade)) {
        $finalgrade = round($graderecord->finalgrade);
    } else {
        $finalgrade = 0;
    }
    $maxgrade = round($graderecord->rawgrademax);
    $allattempts = quiz_get_user_attempts($quiz->id, $USER->id, 'all');
    $lastattempted = end($allattempts);
    $attemptscount = count($allattempts);
    $data['attemptscount'] = $attemptscount;
    $nodates = ($quiz->timeclose == 0 && $quiz->timeopen == 0);
    $hasattemptsleft = ($quiz->attempts == 0 || $attemptscount < $quiz->attempts);
    if (!has_capability('mod/quiz:manage', $modulecontext)) {
        $data['isquiz'] = true;
        if ($attemptscount == 0) {
            $data['yettostart'] = false;
            if ($quiz->timeopen > 0 && $quiz->timeopen > time()) {
                $data['yettostart'] = true;
            }
            if ($quiz->timeclose > 0 && $quiz->timeclose < time()) {
                $data['missed'] = true;
                $data['achievedgrade'] = false;
            }
        } else {
            if (($nodates && $quiz->attempts == 0) 
                || ($nodates && ($quiz->attempts > 0 && $attemptscount < $quiz->attempts))
                || ($hasattemptsleft && $quiz->timeclose > 0 && $quiz->timeclose > time())) {
                $data['missed'] = false;
                $data['achievedgrade'] = $finalgrade.' / '.$maxgrade;
                $data['yettostart'] = false;
            } else if (($nodates && ($quiz->attempts > 0 && $attemptscount == $quiz->attempts))
                || ($quiz->timeclose > 0 && $quiz->timeclose < time() && $hasattemptsleft)
                || ($quiz->timeclose > 0 && $quiz->timeclose > time() && $attemptscount >= $quiz->attempts)) {
                $data['missed'] = false;
                $data['achievedgrade'] = $finalgrade.' / '.$maxgrade;
                $data['yettostart'] = true;
            }
