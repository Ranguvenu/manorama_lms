...
class filter_mediaplugin extends moodle_text_filter {
    ...
    public function filter($text, array $options = array()) {
        ...
        $re = '~<a\s[^>]*href="([^"]*(?:' .  $embedmarkers . '|brighcove_recording_for_streaming_)[^"]*)"[^>]*>([^>]*)</a>~is';
    ...
    private function callback(array $matches) {
        ...
        if(strpos($matches[1], 'brighcove_recording_for_streaming_') === 0){
            if(file_exists($CFG->dirroot.'/local/brightcove/lib.php')){
                require_once($CFG->dirroot.'/local/brightcove/lib.php');
                return local_brightcove_get_video_content($matches[1]);
            }
        }
        // Get name.
        $name = trim($matches[2]);
