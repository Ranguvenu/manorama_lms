 ...
 * Enrol users form.
 *
 * Simple form to search for users and add them using a trial enrolment to this course.
 *
 * @package enrol_trial
...
class enrol_trial_enrol_users_form extends moodleform {
    ...
    public function definition() {
        ...
        if (empty($extendbase)) {
            if (!$extendbase = get_config('enrol_trial', 'enrolstart')) {
        ...
        $basemenu[4] = get_string('now', 'enrol_trial') . ' (' . userdate($now, get_string('strftimedatetimeshort')) . ')';

        $mform->addElement('header', 'main', get_string('enrolmentoptions', 'enrol'));
        $options = array(
            'ajax' => 'enrol_trial/form-potential-user-selector',
            'multiple' => true,
            'courseid' => $course->id,
            'enrolid' => $instance->id,
        ...
        $mform->addElement('autocomplete', 'userlist', get_string('selectusers', 'enrol_trial'), array(), $options);
        ...
        if (has_capability('moodle/cohort:manage', $context) || has_capability('moodle/cohort:view', $context)) {
            ...
            if (!empty($availablecohorts)) {
                ...
                $mform->addElement('cohort', 'cohortlist', get_string('selectcohorts', 'enrol_trial'), $options);
            }
        }

        $roles = get_assignable_roles($context, ROLENAME_BOTH);
        $mform->addElement('select', 'roletoassign', get_string('assignrole', 'enrol_trial'), $roles);
