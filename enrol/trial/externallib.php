 ...
 * @since Moodle 2.2
 */
class enrol_trial_external extends external_api {
    ...
    public static function enrol_users($enrolments) {
        ...
        // Retrieve the trial enrolment plugin.
        $enrol = enrol_get_plugin('trial');
        if (empty($enrol)) {
            throw new moodle_exception('trialpluginnotinstalled', 'enrol_trial');
        }
        ...
        foreach ($params['enrolments'] as $enrolment) {
            ...
            // Check that the user has the permission to trial enrol.
            require_capability('enrol/trial:enrol', $context);
            ...
            if (!array_key_exists($enrolment['roleid'], $roles)) {
                ...
                throw new moodle_exception('wsusercannotassign', 'enrol_trial', '', $errorparams);
            }

            // Check trial enrolment plugin instance is enabled/exist.
            $instance = null;
            $enrolinstances = enrol_get_instances($enrolment['courseid'], true);
            foreach ($enrolinstances as $courseenrolinstance) {
              if ($courseenrolinstance->enrol == "trial") {
            ...
            if (empty($instance)) {
              $errorparams = new stdClass();
              $errorparams->courseid = $enrolment['courseid'];
              throw new moodle_exception('wsnoinstance', 'enrol_trial', $errorparams);
            }
            ...
            if (!$enrol->allow_enrol($instance)) {
                ...
                $errorparams->courseid = $enrolment['courseid'];
                $errorparams->userid = $enrolment['userid'];
                throw new moodle_exception('wscannotenrol', 'enrol_trial', '', $errorparams);
            }
    ...
    public static function unenrol_users($enrolments) {
        ...
        foreach ($params['enrolments'] as $enrolment) {
            ...
            self::validate_context($context);
            require_capability('enrol/trial:unenrol', $context);
            $instance = $DB->get_record('enrol', array('courseid' => $enrolment['courseid'], 'enrol' => 'trial'));
            if (!$instance) {
                throw new moodle_exception('wsnoinstance', 'enrol_trial', $enrolment);
            }
            ...
            if (!$enrol->allow_unenrol($instance)) {
                throw new moodle_exception('wscannotunenrol', 'enrol_trial', '', $enrolment);
            }
