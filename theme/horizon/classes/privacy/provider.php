...
/**
 * Privacy Subsystem implementation for theme_horizon.
 *
...
namespace theme_horizon\privacy;
...
/**
 * The horizon theme stores a user preference data.
 *
 * @copyright  2018 Andrew Nicols <andrew@nicols.co.uk>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class provider implements
    ...
    public static function export_user_preferences(int $userid) {
        ...
        if (isset($draweropenindexpref)) {
            $preferencestring = get_string('privacy:drawerindexclosed', 'theme_horizon');
            if ($draweropenindexpref == 1) {
                $preferencestring = get_string('privacy:drawerindexopen', 'theme_horizon');
            }
        ...
        if (isset($draweropenblockpref)) {
            $preferencestring = get_string('privacy:drawerblockclosed', 'theme_horizon');
            if ($draweropenblockpref == 1) {
                $preferencestring = get_string('privacy:drawerblockopen', 'theme_horizon');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_horizon',
