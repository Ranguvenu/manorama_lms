...
/**
 * Privacy tests for theme_horizon.
 *
 * @package    theme_horizon
 * @category   test
 * @covers     \theme_horizon\privacy\provider
...
class provider_test extends \core_privacy\tests\provider_testcase {
    ...
    public function test_export_user_preferences(string $preference, bool $value, string $expectdescription): void {
        ...
        $exportedpreferences = $writer->get_user_preferences('theme_horizon');
        $this->assertCount(1, (array) $exportedpreferences);
        $this->assertEquals($value, (bool) $exportedpreferences->{$preference}->value);
        $this->assertEquals(get_string($expectdescription, 'theme_horizon'), $exportedpreferences->{$preference}->description);
    }
}
