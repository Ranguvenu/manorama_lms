 ...
 * Custom goal class.
 */
class faq_queries extends dynamic_form
{
    ...
    public function definition()
    {
        ...
        $categorynames = (new controller)->faq_category_list();

        $categorylist = array(null => get_string('select')) + $categorynames;

        if ($id) {
            $faqcategoryid = $DB->get_field('local_faq_queries', 'categoryid', array('id' => $id));
            $faqcategoryname = $DB->get_field('local_faq_categories', 'name', array('id' => $faqcategoryid));

            $mform->addElement('static',  'categoryidstatic', get_string('categoryid', 'local_faq'), $faqcategoryname);

            $mform->addElement('hidden', 'categoryid', $faqcategoryid);
        } else {
            $mform->addElement('select', 'categoryid', get_string('categoryid', 'local_faq'), $categorylist); // Add elements to your form.
            $mform->addRule('categoryid', get_string('categoryid_goalerr', 'local_faq'), 'required', null);
        ...
        $cangoaledit = $DB->record_exists_sql("SELECT id FROM {local_faq_queries} WHERE  id = $id ");

        if ($id > 0  && $cangoaledit) {
            $goalcode = $DB->get_field_sql('SELECT code FROM {local_faq_queries} WHERE id = ' . $id);
        ...
        $mform->addElement('filemanager', 'attachments', get_string('attachments', 'local_faq'), null, array('subdirs' => 0, 'maxfiles' => 1, 'accepted_types' => array('*')));
    ...
    public function validation($data, $files)
    {
        ...
        $goalcode = $DB->get_record_sql("SELECT id, code FROM {local_faq_queries} where code ='{$code}'");
    ...
    public function process_dynamic_submission()
    {
        ...
        if ($data) {
            ...
            $faqdata = (new controller)->create_update_query($data);
        }
    }
    ...
    public function set_data_for_dynamic_submission(): void
    {
        ...
        $context = context_system::instance();
        $id = $this->optional_param('id', 0, PARAM_INT);
        $data = (new controller)->set_faq_query($id);
        if (!empty($data)) {
            if (!empty($data)) {
                $textfieldoptions = array('trusttext' => true, 'subdirs' => true, 'maxfiles' => -1, 'maxbytes' => $CFG->maxbytes, 'context' => $context);
