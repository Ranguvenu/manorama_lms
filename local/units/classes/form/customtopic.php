 ...
 * Custom class
 */
class customtopic extends  dynamic_form {
    ...
    public function definition() {
        ...
        $displayunitname = $DB->get_field_sql("SELECT CONCAT(lh2.name,' / ',lh1.name,' / ',lh.name, ' / ', sub.name,' / ',lu.name,lu.name,' (',lu.code,')') as fullname 
        ...
        $mform->addElement('hidden', 'dname', $displayunitname);
         $displaychaptername = $DB->get_field_sql("SELECT CONCAT(lh2.name,' / ',lh1.name,' / ',lh.name, ' / ', sub.name,' / ',luu.name,' / ',lc.name,lc.name,' (',lc.code,')') as fullname 
                FROM {local_subjects} AS sub
                JOIN {local_hierarchy} lh ON lh.id = sub.classessid AND lh.depth = 3 
                JOIN {local_hierarchy} lh1 ON lh.parent =lh1.id AND lh1.depth = 2
                JOIN {local_hierarchy} lh2 ON lh1.parent = lh2.id AND lh2.depth = 1
        ...
        $unitandchapterids = $DB->get_record('local_topics',['id' => $id]);  
         $displayunitname = $DB->get_field_sql("SELECT CONCAT(lh2.name,' / ',lh1.name,' / ',lh.name, ' / ', sub.name,' / ',lu.name,lu.name,' (',lu.code,')') as fullname 
                FROM {local_subjects} AS sub
                JOIN {local_hierarchy} lh ON lh.id = sub.classessid AND lh.depth = 3 
                JOIN {local_hierarchy} lh1 ON lh.parent =lh1.id AND lh1.depth = 2
        ...
        if ($id > 0) {
        $topiccode = $DB->get_field_sql("SELECT code FROM {local_topics} WHERE id = $id");
        $mform->addElement('text', 'code', get_string('code', 'local_units'), $topiccode);
        $mform->addElement('hidden', 'code', $topiccode);
        } else {
        $mform->addElement('text', 'code', get_string('code', 'local_units')); // Add elements to your form.
        $mform->addRule('code', get_string('code_topicerr', 'local_units'), 'required', null);
    ...
    public function validation($data, $files) {
        ...
        if (strrpos($code, ' ') !== false) {
         ...
         if (trim($name) == "") {
            $errors['name'] = get_string('name_topicerr', 'local_units');
        }
        $topiccode = $DB->get_record_sql("SELECT id, code FROM {local_topics} WHERE code = '{$code}' ");
        if ($topiccode && (empty($data['id']) || $topiccode->id != $data['id'])) {
            $errors['code'] = get_string('topic_codeerr', 'local_units');
        }
        return $errors;
    }
    /**
    ...
    public function process_dynamic_submission() {
        ...
        if ($data) {
            if ($data->id > 0) {
                $topicdata = (new unit)->update_topic($data);
            } else {
                $topicdata = (new unit)->create_topic($data);
            }
        }
    }
    ...
    public function set_data_for_dynamic_submission(): void {
        ...
        if ($id = $this->optional_param('id', 0, PARAM_INT)) {
            $data = $DB->get_record('local_topics', ['id' => $id], '*', MUST_EXIST);
