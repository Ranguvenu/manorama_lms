...
$url = new moodle_url('/local/questionbank/editquestion.php');
if ($id !== 0) {
    $url->param('id', $id);
}
if ($makecopy) {
...
if ($id) {
    ...
    $addurl = new moodle_url('/local/questionbank/editquestion.php', $url->params());
...
if ($wizardnow !== '') {
    $mform = $qtypeobj->next_wizard_form('editquestion.php', $question, $wizardnow, $formeditable);
} else {
    $mform = $qtypeobj->create_editing_form('editquestion.php', $question, $category, $contexts, $formeditable);
}
...
// print_object($toform);die;
if (!$id) {
    $workshop = $DB->get_record('local_questionbank', ['qcategoryid' => $categoryid]);
}else{
    $workshop = $DB->get_record('local_questionbank', ['qcategoryid' => $toform->categoryobject->id]);
}
if ($competencyids = $toform->customfield_competencies) {
    $workshop->competency = $competencyids;
}
$i = 0;
if ($toform->customfield_coursetopics) {
    foreach($toform->customfield_coursetopics as $topic) {
        $question_topics[$i]['names'] = $topic;
        $i++;
    }
}
else {
    $topics = get_qb_mapped_topics($workshop->id);
...
$competency = $qbank->competencies_mapped_to_workshop($workshop);
$mappedCompetencies = $renderer->render_competencies($competency);
$questiontopics = $renderer->render_topics($question_topics);

// Display a heading, question editing form and possibly some extra content needed for
// for this question type.
echo $OUTPUT->header();
echo $mappedCompetencies;
echo $questiontopics;
