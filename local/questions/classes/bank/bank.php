...
class question_finder implements cache_data_source {
    ...
    public function get_questions_from_categories_and_tags_with_usage_counts($categoryids,
            \qubaid_condition $qubaids, $extraconditions = '', $extraparams = array(), $tagids = array(),$goalid,$boardid,$classid,$courseid,$coursetopicsid,$chapterid,$unitid,$conceptid) {
        global $DB;
        list($qcsql, $qcparams) = $DB->get_in_or_equal($categoryids, SQL_PARAMS_NAMED, 'qc');
        ...
        $join   = "JOIN {question_versions} qv ON qv.questionid = q.id
                   ...
                   JOIN {local_qb_questionreview} qbq ON qbq.questionid = q.id ";
        $from   = $from . " " . $join;
        $where  = "qbe.questioncategoryid {$qcsql}
               AND q.parent = 0
               AND qv.status = '$readystatus'
               AND qbq.qstatus = 'publish'
               AND qv.version = (SELECT MAX(v.version)
                                 ...
                                 WHERE be.id = qbe.id)";
        if($goalid){
           $where  .= " AND lqc.goalid = '$goalid'";
        } 
        if($boardid){
           $where  .= " AND lqc.boardid = '$boardid'";
        }
        if($classid){
           $where .= " AND lqc.classid = '$classid'" ;
        }
        if($courseid){
           $where .= " AND lqc.courseid = '$courseid'" ;
        }
        if ($coursetopicsid) {
            $coursetopicsid = explode(',', $coursetopicsid);
                $cotopquery = array();
                foreach ($coursetopicsid as $ctids) {
                    $ctids= (int)$ctids;
                    $cotopquery[] = " CONCAT(',',lqc.topicid,',') LIKE CONCAT('%,',$ctids,',%') ";
                }
                $qidparamas =implode('OR',$cotopquery);
                $where .= '  AND ('.$qidparamas.') ';
         }
         if($chapterid){
           $where .= " AND lqc.chapterid = '$chapterid'" ;
        }
        if($unitid){
           $where .= " AND lqc.unitid = '$unitid'" ;
        }
        if($conceptid){
           $where .= " AND lqc.conceptid = '$conceptid'" ;
        }
        $params = $qcparams;
