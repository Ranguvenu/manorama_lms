...
//use local_questions\structure;
use \html_writer;
use qbank_previewquestion\question_preview_options;
use renderable;
//use \local_questions\bank\structure as structure;
/**
 * Renderer outputting the quiz editing UI.
 *
 * @copyright 2013 The Open University.
...
class edit_renderer extends \plugin_renderer_base {
    ...
    public function random_question(structure $structure, $slotnumber, $pageurl) {
        ...
        $temp = clone($question);
        $temp->questiontext = '';
        $temp->name = \local_questions\bank\qbankhelper::describe_random_question($slot);
        ...
        if ($slot->goalid) {
           $goalid=array_keys($slot->goalid);
            $qbankurlparams["goalid"] = $goalid[0];
        }
        if ($slot->boardid) {
           $boardid=array_keys($slot->boardid);
            $qbankurlparams["boardid"] = $boardid[0];
        }
        if ($slot->classid) {
           $classid=array_keys($slot->classid);
            $qbankurlparams["classid"] = $classid[0];
        }
        if ($slot->courseid) {
           $courseid=array_keys($slot->courseid);
            $qbankurlparams["subjectid"] = $courseid[0];
        }
        if ($slot->coursetopicid) {
           $coursetopicid=array_keys($slot->coursetopicid);
            $qbankurlparams["topicid"] = $coursetopicid[0];
        }
         if ($slot->chapterid) {
           $chapterid=array_keys($slot->chapterid);
            $qbankurlparams["chapterid"] = $chapterid[0];
        }
         if ($slot->unitid) {
           $unitid=array_keys($slot->unitid);
            $qbankurlparams["unitid"] = $unitid[0];
        }
         if ($slot->conceptid) {
           $conceptid=array_keys($slot->conceptid);
            $qbankurlparams["conceptid"] = $conceptid[0];
        }
        $qbankurlparams["qstatus"] = 'publish';
        ...
        $qbankurl = new \moodle_url('/local/questions/questionbank_view.php', $qbankurlparams);
     ...
     */
    public function question_bank_contents(\local_questions\local\qselection_view $questionbank, array $pagevars) {
