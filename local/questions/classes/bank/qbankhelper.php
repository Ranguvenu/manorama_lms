...
class qbankhelper {
    ...
    public static function get_question_structure(int $quizid, \context_module $quizcontext,
        ...
        foreach ($slotdata as $slot) {
            ...
            if ($slot->filtercondition) {
                ...
                $slot->goalid = isset($filtercondition->goalid) ? (array) $filtercondition->goalid : [];
                $slot->boardid = isset($filtercondition->boardid) ? (array) $filtercondition->boardid : [];
                $slot->classid = isset($filtercondition->classid) ? (array) $filtercondition->classid : [];
                $slot->courseid = isset($filtercondition->courseid) ? (array) $filtercondition->courseid : [];
                $slot->coursetopicid = isset($filtercondition->coursetopicid) ? (array) $filtercondition->coursetopicid : [];
                $slot->chapterid = isset($filtercondition->chapterid) ? (array) $filtercondition->chapterid : [];
                $slot->unitid = isset($filtercondition->unitid) ? (array) $filtercondition->unitid : [];
                $slot->conceptid = isset($filtercondition->conceptid) ? (array) $filtercondition->conceptid : [];
    ...
    public static function describe_random_question(\stdClass $slotdata): string {
        ...
        return self::question_name(
               $category, $slotdata->randomrecurse, $slotdata->randomtags, $slotdata->goalid, $slotdata->boardid, $slotdata->classid, $slotdata->courseid, $slotdata->classid, $slotdata->courseid, $slotdata->coursetopicid, $slotdata->chapterid, $slotdata->unitid,$conceptid);
     ...
     */
    public static function question_name($category, $includesubcategories, $tagnames = [],$goalname = [], $boardname= [], $classname = [], $coursename = [], $courstopicname = [], $chaptername = [], $unitname = [], $conceptname = []) {
        $categoryname = '';
        if ($category->parent && $includesubcategories) {
            $a = shorten_text($category->name, 100);
            $stringarr[] = get_string('randomqplusname', 'local_questions',$a);
        } else if ($category->parent) {
            $a = shorten_text($category->name, 100);
            $stringarr[] = get_string('randomqname', 'local_questions',$a);
            ...
            switch ($context->contextlevel) {
                ...
                case CONTEXT_COURSECAT:
                    ...
                    $stringarr[] = get_string('randomqplusnamecoursecat', 'local_questions',$categoryname);
                    break;
                case CONTEXT_SYSTEM:
        ...
        if($goalname){
           $gname =array_values($goalname);
            $a = $gname[0];
            $stringarr[] = get_string('goal', 'local_questions',$a);
        }
         if($boardname){
            $bname =array_values($boardname);
            $a = $bname[0];
            $stringarr[] = get_string('board', 'local_questions',$a);
        }
        
         if($classname){
            $cname = array_values($classname);
            $a = $cname[0];
            $stringarr[] = get_string('class', 'local_questions',$a);
        }
          if($coursename){
           $couname = array_values($coursename);
            $a = $couname[0];
            $stringarr[] = get_string('subject', 'local_questions',$a);
      
        }
          if($courstopicname){
            $coutname = array_values($courstopicname);
            $a=$coutname[0];
            $stringarr[] = get_string('coursetopic', 'local_questions',$a);
        }
          if($chaptername){
           $chaname = array_values($chaptername);
            $a = $chaname[0];
            $stringarr[] = get_string('chapter', 'local_questions',$a);
      
        }
          if($unitname){
           $unname = array_values($unitname);
            $a = $unname[0];
            $stringarr[] = get_string('unit', 'local_questions',$a);
      
        }
          if($conceptname){
           $conname = array_values($conceptname);
            $a = $conname[0];
            $stringarr[] = get_string('concept', 'local_questions',$a);
      
        }

        $stringvalue= implode(', ',$stringarr);
        $name = $stringvalue;
        //$name = get_string($stringvalue, 'local_questions', $a);
