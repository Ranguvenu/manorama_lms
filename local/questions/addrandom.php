...
use local_questions\form\addrandom_form;
use mod_quiz\quiz_settings;
use qbank_managecategories\question_category_object;

list($thispageurl, $contexts, $cmid, $cm, $quiz, $pagevars) =
        question_edit_setup('editq', '/local/questions/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/local/questions/addrandom.php', ['cmid' => $cmid]);
}
if ($mdlscrollto) {
...
$mform = new addrandom_form(new moodle_url('/local/questions/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    $goalid = $data->goal ? $data->goal : 0 ;
    $boardid = $data->board ? $data->board : 0 ;
    $classid = $data->class ? $data->class : 0 ;
    $courseid = $data->course ? $data->course : 0 ;
    $coursetopicid = $data->coursetopics ? $data->coursetopics : 0 ;
    $chapterid = $data->chapter ? $data->chapter : 0 ;
    $unitid = $data->unit ? $data->unit : 0 ;
    $conceptid = $data->concept ? $data->concept : 0 ;

    local_questions_quiz_add_random_questions($quiz, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories, $tagids, $goalid, $boardid, $classid, $courseid, $coursetopicid,$chapterid,$unitid,$conceptid);
