...
class notification
{
    ...
    public static function get_recipient_label($nbdest, $availability, $msgbodyinfo) {
        if ($nbdest == 0) {
            return get_string('norecipient', 'local_notifications');
        }
        if ($availability) {
            $a = new stdClass();
            $a->nbdest = $nbdest;
            ...
            return get_string('grouprecipient', 'local_notifications', $a);
        } else {
            return get_string('allstudentrecipient', 'local_notifications', $nbdest);
        }
    }
    ...
    private static function get_result_action_notification($infolog) {
        if ($infolog['nb'] == 0) {
            return get_string('nomessagesend', 'local_notifications');
        }
        $message = get_string('numbernotification', 'local_notifications', $infolog['nb']);
        return $message;
    }
    ...
    private static function send_email($user, $msg, $courseid) {
        ...
        $eventdata->name = 'notifications_notification';
    ...
    public static function get_email_subject($courseshortname, $activitename) {
        $subject = get_string('notification', 'local_notifications')
    ...
    public static function get_email_body($msgbodyinfo, $type) {
        $message_body = get_config('local_notifications','message_body');
