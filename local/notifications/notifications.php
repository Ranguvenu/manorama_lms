...
$url = new moodle_url('/local/notifications/notifications.php', array('id'=>$id));
$PAGE->set_url($url);
...
$PAGE->requires->css(new moodle_url('/local/notifications/notifications.css'));
...
if ($nbNotifiedStudents == 0) {
    $recipients = get_string('norecipient', 'local_notifications');
} else {
...
$mform = new local_notifications_notifications_form(null, $infoform);
...
echo $OUTPUT->heading(get_string('sendnotification', 'local_notifications'));

if ($msgresult != '') {
    echo $OUTPUT->box_start('info');
    echo $msgresult;
    echo html_writer::tag('p', html_writer::link($urlcourse, get_string('returncourse', 'local_notifications')));
    echo $OUTPUT->box_end();
} else {
    $mform->display();
}
