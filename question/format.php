...
class qformat_default {
    ...
    public function importprocess() {
        ...
        foreach ($questions as $question) {
            $currentquestion = json_encode($question);
        ...
        foreach ($questions as $question) {   // Process and store each question
            ...
            if (isset($question->idnumber)) {
                if ((string) $question->idnumber === '') {
                    ...
                    if ($DB->record_exists('question_bank_entries',
                        ...
                        //unset($question->idnumber);
                        $getidnumber = $DB->get_record('question_bank_entries',['idnumber' => $question->idnumber]);
                       // $getqidsql = " SELECT MAX(questionid) AS questionid FROM {question_versions} WHERE questionbankentryid = :questionentryid ";
                         $getqidsql =" SELECT MAX(questionid) AS questionid FROM {question_versions} WHERE (SELECT max(version) from {question_versions}) AND questionbankentryid = :questionentryid ";
                        $getqid = $DB->get_record_sql($getqidsql, ['questionentryid' => $getidnumber->id]);
                        $getqversionsql = " SELECT version FROM {question_versions} WHERE questionid = :qstid ";
                        $getversion = $DB->get_record_sql($getqversionsql, ['qstid' => $getqid->questionid]);
                        $questiontypesql = " SELECT qtype FROM {question} WHERE id = :qid ";
                        $questiontype = $DB->get_record_sql($questiontypesql, ['qid' => $getqid->questionid]);
                        $addversion = 0;
                        if($questiontype->qtype == $question->qtype){
                           $addversion = (int)$getversion->version;
                           $qbentry = false;
            ...
            if($qbentry){
                $questionbankentry->id = $DB->insert_record('question_bank_entries', $questionbankentry);
                 ...
                 $questionbankentry->id = $getidnumber->id; 
            }
            ...
            if($addversion != 0){
            $questionversion->version = 1 + $addversion; 
            }else{
            ...
            // $customfieldobj = new \customfield_coursetopics\data_controller($question->id, $question);
            ...
            $customfieldhandler->instance_form_save($question);
            ...
            $classname = '\local_questions\questionslog';
            if(class_exists($classname)){
                $obj = new \local_questions\questionslog(); 
                $logstatus = 1;
                if (!empty($result->error)) {
                    $logstatus = 0;
                }
                $questionlogs = $obj->save_question_logs($question,$logstatus,$currentquestion);
            }
