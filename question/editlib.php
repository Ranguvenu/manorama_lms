...
function question_edit_setup($edittab, $baseurl, $requirecmid = false, $unused = null) {
    ...
    if(optional_param('goalid', null, PARAM_INT)){
    $params['goalid'] = optional_param('goalid', null, PARAM_INT);     
    }
    if(optional_param('boardid', null, PARAM_INT)){
    $params['boardid'] = optional_param('boardid', null, PARAM_INT);     
    }
    if(optional_param('classid', null, PARAM_INT)){
    $params['classid'] = optional_param('classid', null, PARAM_INT);     
    }
    if(optional_param('subjectid', null, PARAM_INT)){
    $params['subjectid'] = optional_param('subjectid', null, PARAM_INT);     
    }
    if(optional_param('topicid', null, PARAM_INT)){
    $params['topicid'] = optional_param('topicid', null, PARAM_INT);     
    }
     if(optional_param('chapterid', null, PARAM_INT)){
    $params['chapterid'] = optional_param('chapterid', null, PARAM_INT);     
    }
     if(optional_param('unitid', null, PARAM_INT)){
    $params['unitid'] = optional_param('unitid', null, PARAM_INT);     
    }
    if(optional_param('conceptid', null, PARAM_INT)){
    $params['conceptid'] = optional_param('conceptid', null, PARAM_INT);     
    }
    if(optional_param('difficulty', null, PARAM_INT)){
    $params['difficulty'] = optional_param('difficulty', null, PARAM_INT);     
    }
    if(optional_param('cognitive', null, PARAM_INT)){
    $params['cognitive'] = optional_param('cognitive', null, PARAM_INT);     
    }
    if(optional_param('source', null, PARAM_INT)){
    $params['source'] = optional_param('source', null, PARAM_INT);     
    }
    if(optional_param('qstatus', null, PARAM_RAW)){
    $params['qstatus'] = optional_param('qstatus', null, PARAM_RAW);     
    }
    if(optional_param('uploadfrom', null, PARAM_INT)){
        $optparamufrom =optional_param('uploadfrom', null, PARAM_INT);
         if(is_array($optparamufrom)){
            $start_year =$optparamufrom['year'];
            $start_month = $optparamufrom['month'];
            $start_day = $optparamufrom['day'];
            $params['uploadfrom'] = mktime(0,0,0, $start_month, $start_day, $start_year);
          }else{
         $params['uploadfrom'] = optional_param('uploadfrom', null, PARAM_INT); 
        }
    }
    if(optional_param('uploadto', null, PARAM_INT)){
         $optparamuto =optional_param('uploadto', null, PARAM_INT);
          if(is_array($optparamuto)){
            $start_yearto =$optparamuto['uploadto']['year'];
            $start_monthto = $optparamuto['uploadto']['month'];
            $start_dayto = $optparamuto['uploadto']['day'];
            $params['uploadto'] = mktime(0,0,0, $start_monthto, $start_dayto, $start_yearto);
        }else{
        $params['uploadto'] = optional_param('uploadto', null, PARAM_INT); 
        }    
    }
...
function question_build_edit_resources($edittab, $baseurl, $params,
    ...
    $paramtypes = [
        ...
        'unitid' =>PARAM_INT,
        'conceptid' =>PARAM_INT,
        'difficulty' =>PARAM_INT,
        'cognitive' =>PARAM_INT,
        'source' =>PARAM_INT,
        'qstatus' =>PARAM_RAW,
        'uploadfrom' =>PARAM_INT,
        'uploadto' =>PARAM_INT,
    ];
    ...
    $goalid = $cleanparams['goalid'];
    $boardid = $cleanparams['boardid'];
    $classid = $cleanparams['classid'];
    $subjectid = $cleanparams['subjectid'];
    $topicid = $cleanparams['topicid'];
    $chapterid = $cleanparams['chapterid'];
    $unitid = $cleanparams['unitid'];
    $conceptid = $cleanparams['conceptid'];
    $difficulty = $cleanparams['difficulty'];
    $cognitive = $cleanparams['cognitive'];
    $source = $cleanparams['source'];
    $qstatus = $cleanparams['qstatus'];
    $uploadfrom = $cleanparams['uploadfrom'];
    $uploadto = $cleanparams['uploadto'];
    ...
    if($difficulty){
        $pagevars['difficulty'] = $difficulty; 
    }
    if($cognitive){
        $pagevars['cognitive'] = $cognitive; 
    }
    if($source){
        $pagevars['source'] = $source; 
    }
    if($qstatus){
        $pagevars['qstatus'] = $qstatus; 
    }
    if($uploadfrom){
    ...
    if ($pagevars['cat']){
      ...
      if ($pagevars['goalid']){
        $thispageurl->param('goalid', $pagevars['goalid']);
    }
      if ($pagevars['boardid']){
        $thispageurl->param('boardid', $pagevars['boardid']);
    }
      if ($pagevars['classid']){
        $thispageurl->param('classid', $pagevars['classid']);
    }
     if ($pagevars['topicid']){
        $thispageurl->param('topicid', $pagevars['topicid']);
    }
    if ($pagevars['chapterid']){
        $thispageurl->param('chapterid', $pagevars['chapterid']);
    }
    if ($pagevars['unitid']){
        $thispageurl->param('unitid', $pagevars['unitid']);
    }
     if ($pagevars['conceptid']){
        $thispageurl->param('conceptid', $pagevars['conceptid']);
    }
     if ($pagevars['subjectid']){
        $thispageurl->param('subjectid', $pagevars['subjectid']);
    }
    if ($pagevars['difficulty']){
        $thispageurl->param('difficulty', $pagevars['difficulty']);
    }
    if ($pagevars['cognitive']){
        $thispageurl->param('cognitive', $pagevars['cognitive']);
    }
     if ($pagevars['source']){
        $thispageurl->param('source', $pagevars['source']);
    }
    if ($pagevars['qstatus']){
        $thispageurl->param('qstatus', $pagevars['qstatus']);
    }
     if ($pagevars['uploadto']){
        $thispageurl->param('uploadto', $pagevars['uploadto']);
    }
     if ($pagevars['uploadfrom']){
        $thispageurl->param('uploadfrom', $pagevars['uploadfrom']);
    }
