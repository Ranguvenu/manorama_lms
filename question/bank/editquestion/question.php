...
if ($cmid) {
    ...
    $PAGE->navbar->add(get_string('pluginname', 'local_questions'), new moodle_url('/local/questions/index.php'));
    $questionbankurl = new moodle_url('/local/questions/index.php');
}
...
if ($mform->is_cancelled()) {
    ...
    //For Copying the Question Hierarchy
      if (!empty($fromform->updatebuttoncopy)) {
        $url->param('goalid', $fromform->customfield_goal);
        $url->param('boardid', $fromform->customfield_board);
        $url->param('classid', $fromform->customfield_class);
        $url->param('subjectid', $fromform->customfield_courses);
        $url->param('unitid', $fromform->customfield_unit);
        $url->param('chapterid', $fromform->customfield_chapter);
        $url->param('topicsid', $fromform->customfield_topics);
        $url->param('conceptid', $fromform->customfield_concept);
        $url->param('difficultyid', $fromform->customfield_difficultylevel);
        $url->param('cognitiveid', $fromform->customfield_cognitivelevel);
        $url->param('sourceid', $fromform->customfield_source);
        $url->remove_params('makecopy');
        redirect($url);
    }
