    ...
    public function get_icon_url($output = null) {
        ...
        if (!empty($this->iconurl)) {
            ...
            if($this->modname == 'quiz'){
                $testtype = $DB->get_field($this->modname, 'testtype', ['id' => $this->instance]);
                if($testtype){
                    $icon = $output->image_url('quizact', $this->modname);
                }else{
                    $icon = $output->image_url('textact', $this->modname);
                }
            } else if($this->modname == 'page'){
                $pagetype = $DB->get_field($this->modname, 'pagetype', ['id' => $this->instance]);
                if($pagetype){
                    $icon = $output->image_url('playact', $this->modname);
                }else{
                    $icon = $output->image_url('pageact', $this->modname);
                }
            }else{
