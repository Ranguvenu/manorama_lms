...
class core_webservice_external extends \core_external\external_api {
    ...
    public static function get_site_info($serviceshortnames = array()) {
        ...
        //Added by renu for getting usertype.
        $params = [];  
        ...
        $courses = $DB->count_records_sql($sql, $params);

        if ($courses > 0) {
            $user_type = get_string('purchased', 'webservice');
        } else {
            $user_type = get_string('registered', 'webservice');
        }
        ...
        if ($CFG->enablewebservices) { // No need to check token if web service are disabled and not a ws call.
            ...
            if (!empty($token)) { // No need to run if not a ws call.
                ...
                $service = $DB->get_record_sql($servicesql, $array);
                ...
                if (!empty($service)) {
                    ...
                    $functionssql = "SELECT f.*
                                      ...
                                      WHERE f.name = sf.functionname
                                        AND sf.externalserviceid = :externalserviceid";
                    $functions = $DB->get_records_sql($functionssql, ['externalserviceid' => $service->id]);
                } else {
        ...
        $configdata = get_config('theme_horizon');
        if (!empty($configdata->aboutusurl)) {
            $aboutusurl = $configdata->aboutusurl;
        } else {
            $aboutusurl = 'https://manoramal.eabyas.in:1443/about-us';
        }
        $siteinfo['aboutus'] = $aboutusurl;
    ...
    public static function get_site_info_returns() {
        return new external_single_structure(
            array(
                ...
                'usertype'       => new external_value(PARAM_RAW, 'usertype'),
                ...
                'aboutus' => new external_value(PARAM_URL, 'For the about-us details', VALUE_OPTIONAL),
            )
        );
    }
}
