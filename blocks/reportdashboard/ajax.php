...
$goalid = optional_param('goalid', $requests['goalid'], PARAM_INT);
$boardid = optional_param('boardid', $requests['boardid'], PARAM_INT);
$classid = optional_param('classid', $requests['classid'], PARAM_INT);
$subjectid = optional_param('subjectid', $requests['subjectid'], PARAM_INT);
...
switch ($action) {
    ...
    case 'updatedashboard':
        ...
        case 'boardslist':
            if (!empty($goalid)) {
                $boards = $DB->get_records_sql_menu("SELECT id, name
                            FROM {local_hierarchy}
                            WHERE parent = :parentid AND depth = :depth",
                            ['parentid' => $goalid, 'depth' => 2]);

                if (!empty($boards)) {
                    $return = $boards;
                } else {
                    $return = array('0' => 'Select board');
                }
            } else {
                $return = array('0' => 'Select board');
            }
        ...
        case 'classeslist':
            if (!empty($boardid)) {
                $classes = $DB->get_records_sql_menu("SELECT id, name
                            FROM {local_hierarchy}
                            WHERE parent = :parentid AND depth = :depth",
                            ['parentid' => $boardid, 'depth' => 3]);

                if (!empty($classes)) {
                    $return = $classes;
                } else {
                    $return = array('0' => 'Select class');
                }
            } else {
                $return = array('0' => 'Select class');
            }
        ...
        case 'subjectslist':
            if (!empty($classid)) {
                $subjects = $DB->get_records_sql_menu("SELECT id, name
                            FROM {local_subjects}
        ...
        case 'batcheslist':
            if (!empty($subjectid)) {
                $batches = $DB->get_records_sql_menu("SELECT lb.id, lb.name
        ...
        case 'chapterslist':
            if (!empty($courseid)) {
                $chapters = $DB->get_records_sql_menu("SELECT DISTINCT cf.id,
                ...
                AND cs.course = :chaptercourseid", ['chaptercourseid' => $courseid]);

                if (!empty($chapters)) {
                    $return = array('0' => 'Select chapter') + $chapters;
                } else {
                    $return = array('0' => 'Select chapter');
                }
            } else {
                $return = array('0' => 'Select chapter');
            }
