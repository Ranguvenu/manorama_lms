...
class block_learnerscript_observer {
    ...
    public static function page_completion(\core\event\course_module_viewed $event){
		global $DB, $CFG;
        try {
            ...
            $record->page = $data['contextinstanceid'];
            $record->viewed = 1;
            if(!($DB->record_exists("block_ls_pageviewed", ['userid' => $data['userid'], 'page' => $record->page]))){
                $DB->insert_record("block_ls_pageviewed", $record);
            }
        } catch (Exception $e) {
            debugging($e->getMessage());
        }
