...
class ls {
	...
	public function generate_report_plot($reportclass, $graphdata, $blockinstanceid = null) {
			...
			if (!empty($seriesvalues)) {
				...
				case 'column':
					...
					break;
				case 'stacked':
					return $highcharts->lbchart($reportclass->finalreport->table->data, $graphdata, $reportclass->config, 'stacked', $blockinstanceid, $reportclass->finalreport->table->head);
					break;
				case 'combination':
	...
	public function strTime($values) {
      	...
      	$values -= $minutes*60;
      	if (!empty($hours)) {
      		$hrs = ($hours == 1) ? round($hours) . 'Hr ' : round($hours) . 'Hrs ';
      	} else {
      		$hrs = '';
      	}

      	if (!empty($minutes)) {
      		$min = round($minutes) . 'Min ';
      	} else {
      		$min = '';
      	}

      	if (!empty($values)) {
      		$sec = round($values). 'Sec';
      	} else {
      		$sec = '';
      	}
 	...
 	public function get_subsections($courseid, $sectionid, $sectionids) { 
		global $DB, $USER; 
	
		$childsections = $DB->get_fieldset_sql("SELECT cfo.sectionid FROM {course_format_options} cfo JOIN {course_sections} cs ON cs.section = cfo.value WHERE cs.id = :sectionid AND cfo.format like 'flexsections' AND cfo.name like 'parent' AND cfo.value > 0 AND cfo.courseid = :courseid ", ['sectionid' => $sectionid,  'courseid' => $courseid]);
		
		if (!empty($childsections)) {
			foreach ($childsections as $csection) {
				$subsection = $this->get_subsections($courseid, $csection, $sectionids);
				$sectionids = array_merge($sectionids, $subsection);
			}
		}
	...
	public function get_parent_chapter($courseid, $sectionid) { 
		global $DB, $USER; 
	
		$sectionids = [];
		$topic = $DB->get_record_sql("SELECT IF(cf.value=0, (SELECT name FROM mdl_course_sections WHERE id = cf.sectionid AND course = $courseid), cs.name) AS name, cf.value, IF(cf.value=0, cf.sectionid, cs.id) AS id
		FROM mdl_course_sections cs 
		JOIN mdl_course_format_options cf ON cs.section = cf.value AND cf.courseid = $courseid
		WHERE cf.sectionid = $sectionid AND cf.name like 'parent' AND cf.format LIKE 'flexsections' AND cs.course = $courseid");
		if($topic->value == 0){
			return $topic->id;
		}
		return $this->get_parent_chapter($courseid, $topic->id);
	}
