...
function block_learnerscript_plotforms_ajaxform($args) {
    ...
    if(array_search($args->pname, ['bar', 'column', 'line', 'stacked'])){
        $pname = 'bar';
    }else{
        $pname = $args->pname;
    }
...
// function get_section_performance($courseid, $sectionid) { 
//     global $DB, $USER; 

//     $sectionids = [];
//     $childsections = $DB->get_fieldset_sql("SELECT cfo.sectionid FROM {course_format_options} cfo JOIN {course_sections} cs ON cs.section = cfo.value WHERE cs.id = :sectionid AND cfo.format like 'flexsections' AND cfo.name like 'parent' AND cfo.courseid = :courseid ", ['sectionid' => $sectionid,  'courseid' => $courseid]);
    
//     if (!empty($childsections)) {
//         foreach ($childsections as $csection) {
//             $subsection = get_section_performance($courseid, $csection);
