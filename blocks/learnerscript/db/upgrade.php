...
function xmldb_block_learnerscript_upgrade($oldversion) {
    ...
    if ($oldversion < 2021051705.6) {
        $dbman = $DB->get_manager();
        $cmpltn_table = new xmldb_table('page_completion');
        $table = new xmldb_table('block_ls_pageviewed');
        if ($dbman->table_exists($cmpltn_table)) {
            $field = new xmldb_field('completed');
            $dbman->drop_field($cmpltn_table, $field);
            $dbman->rename_table($cmpltn_table, 'block_ls_pageviewed');
        } else if(!$dbman->table_exists($table)){
            $table = new xmldb_table('block_ls_pageviewed');
            $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE);
            $table->add_field('userid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, '0');
            $table->add_field('page', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, '0');
            $table->add_field('viewed', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, '0');
            $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
            $dbman->create_table($table);
        }

        upgrade_plugin_savepoint(true, 2021051705.6,'', 'block_ls_pageviewed');
    }   
    return true;
}
