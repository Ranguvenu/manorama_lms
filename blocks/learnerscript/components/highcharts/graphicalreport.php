...
class highreports {
	...
	*/
	...
	public function lbchart($data, $series, $name, $type, $containerid = null, $head) {
		...
		$containerid == null ? $containerid = $series['id'] : null;
		empty($series['formdata']->serieslabel) ? $series['formdata']->serieslabel = $name->name : null;
		$lbchartdata = $this->get_lbchartdata($data, $series, $type, $head, $name, $stacking);
	...
	public function get_lbchartdata($data, $series, $type, $head, $report, $stacking) {
		global $CFG;
		$i = 0;
		$error = array();
		$graphdata = array();
				...
				if (array_key_exists($yaxis, $head)) {
					if ($data) {
						...
						foreach ($data as $r) {
							...
							if (empty($series['formdata']->calcs)) {
								if(!empty($stacking) && $stacking == 'normal') {
