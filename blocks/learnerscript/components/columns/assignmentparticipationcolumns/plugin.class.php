  ...
  public function execute($data,$row,$user,$courseid,$starttime=0,$endtime=0,$reporttype){
    ...
    switch ($data->column) {
        case 'username':
            ...
            break;
        case 'submitteddate':
            $submitteddate =  $DB->get_field_sql("SELECT asb.timemodified FROM {assign} a JOIN {assign_submission} asb ON asb.assignment = a.id WHERE asb.userid = :userid AND a.id = :assignmentid AND asb.status = 'submitted'", ['userid' => $row->userid,'assignmentid' => $row->id]);
        ...
        case 'finalgrade':
            ...
            $finalgrade = $DB->get_field_sql("SELECT gg.finalgrade AS finalgrade 
                           ...
                           WHERE 1 = 1 AND gi.itemmodule = 'assign' AND gg.userid = :userid AND gi.iteminstance = :assignid",['userid' => $row->userid,'assignid' => $row->id]);
              if(!empty($finalgrade)){
