...
class activityfield_form extends moodleform {

    public function definition() {
        global $DB, $USER, $CFG;
        $mform = & $this->_form;
        $mform->addElement('header', 'crformheader', get_string('activityfield', 'block_learnerscript'), '');
        $columns = $DB->get_columns('course_modules');
        $activitycolumns = array();
        foreach ($columns as $c) {
            $activitycolumns[$c->name] = ucfirst($c->name);
        }
        unset($activitycolumns['availability']);
