...
use moodle_url;
use DateTime;
class plugin_testscoredetailscolumns extends pluginbase{
  public function init(){
    $this->fullname = get_string('testscoredetailscolumns','block_learnerscript');
    $this->type = 'undefined';
    $this->form = true;
    $this->reporttypes = array('testscoredetails');
  }
    ...
    switch ($data->column) {
            ...
            case 'startdate':
                if(isset($row->startdate) && !empty($row->startdate)) {
                    ...
                    $row->{$data->column} = $startdate.' '.$starttime;
                } else {
                    $row->{$data->column} = 'NA';
                }
            break;
            case 'enddate':
                if(isset($row->startdate) && !empty($row->startdate)) {
                    ...
                    $row->{$data->column} = $duedate.' '.$duetime;
            ...
            case 'status':
                if(isset($row->status) && !empty($row->status)){
                    ...
                    if($row->modulename == 'assign') {
                        ...
                        if(!empty($submission)){
                           ...
                           WHERE assignment = :activityid AND userid = :userid", ['userid' => $row->userid, 'activityid' => $row->activityid]);
                                if($started) {
                         ...
                         if($submitteddate) {
                            ...
                            if(!empty($row->duedate)) {
                                 if($timestamp > $row->duedate) {
                                    ...
                                    $started =  $DB->get_field_sql("SELECT qa.id FROM {quiz_attempts} qa WHERE qa.quiz = :quizid AND qa.userid=:userid ORDER BY attempt DESC LIMIT 1", ['userid' => $row->userid,'quizid' => $row->activityid]);
                                    if ($started) {
                                        $status = get_string('inprogress', 'block_learnerscript');
                                    } else {
                                        $status = get_string('notstarted', 'block_learnerscript');
                ...
                break;
            case 'url':
                if(isset($row->instanceid) && !empty($row->instanceid)){
                     $url = new moodle_url('/mod/'.$row->modulename.'/view.php',
                                   array('id' => $row->instanceid));
                    if($row->modulename == 'quiz') {
                        if($row->status == 'Submitted') {
                            $attemptid = $DB->get_field_sql("SELECT id FROM {quiz_attempts} WHERE quiz = :quizid AND userid = :userid ORDER BY attempt DESC LIMIT 1", ['quizid' => $row->activityid, 'userid' => $row->userid]);
                            $url = new moodle_url('/mod/'.$row->modulename.'/review.php',
                                   array('attempt' => $attemptid,'cmid' => $row->instanceid));
