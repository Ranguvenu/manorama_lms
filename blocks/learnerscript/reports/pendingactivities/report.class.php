    ...
    function select() {
        ...
        foreach ($aliases as $alias) {
            ...
            if($alias == 'assign'){
                ...
                JOIN {user} u ON u.id = ue.userid 
                JOIN {".$alias."} AS $alias ON $alias.id = main.instance AND m.name = '$alias' AND $alias.duedate < $timestamp
                JOIN {assign_submission} AS asb ON asb.assignment= $alias.id AND asb.userid =$userid WHERE m.visible = 1 AND m.name IN ($activitynames) AND main.visible = 1 AND asb.status != 'submitted'".$filters;
            }
            if($alias == 'quiz'){
