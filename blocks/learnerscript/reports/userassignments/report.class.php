...
class report_userassignments extends reportbase implements report {
    ...
    public function column_queries($columnname, $assignid, $courseid = null) {
        ...
        switch ($columnname) {
                                 ...
                                 WHERE cm.visible = 1 AND cm.deletioninprogress = 0 AND m.name = 'assign'
                                   ...
                                   AND cm.instance NOT IN (SELECT cm.instance
                                                            ...
                                                            WHERE cm.visible = 1 AND cm.deletioninprogress = 0 AND c.visible = 1
                                                              AND m.name = 'assign' AND cmc.completionstate <> 0  AND cm.course IN ($filtercourseid) $where
                              ...
                              WHERE cm.visible = 1 AND c.visible = 1 AND m.name = 'assign'
                                AND cm.instance NOT IN
                                    (SELECT cm.instance
                                      ...
                                      WHERE cm.visible = 1 AND cm.deletioninprogress = 0 AND c.visible = 1 AND m.name = 'assign' AND cm.course IN ($filtercourseid) AND cmc.completionstate > 0 $where)  AND cm.course IN ($filtercourseid) $where ";
            break;
            case 'completed':
                $identy = 'cmc.userid';
            ...
            case 'totaltimespent':
                $identy = 'mt.userid';
                $query = " SELECT SUM(mt.timespent) AS totaltimespent FROM {block_ls_modtimestats} AS mt JOIN {course_modules} cm ON cm.id = mt.activityid JOIN {modules} m ON m.id = cm.module WHERE m.name = 'assign' AND cm.course IN ($filtercourseid) $where ";
            break;
            case 'numviews':
                $identy = 'lsl.userid';
                $query = "SELECT COUNT(DISTINCT lsl.id) AS numviews 
                                        ...
                                        WHERE m.name = 'assign' AND lsl.crud = 'r' AND lsl.contextlevel = 70 AND lsl.anonymous = 0 AND cm.course IN ($filtercourseid) $where";
            break; 
