...
class report_sql extends reportbase {
    ...
    function prepare_sql($sql) {
        ...
        if (preg_match("/%%BATCHCOURSE:([^%]+)%%/i", $sql, $output) && isset($this->params['filter_courses'])) {
            $courseid = !empty($this->params['filter_courses']) ? $this->params['filter_courses'] : 0;
            $replace = ' AND ' . $output[1] . ' = ' . $courseid;
            $sql = str_replace('%%BATCHCOURSE:' . $output[1] . '%%', $replace, $sql);
        }
        if (preg_match("/%%BATCH_COURSE:([^%]+)%%/i", $sql, $output) && isset($this->params['filter_courses'])) {
            $courseid = !empty($this->params['filter_courses']) ? $this->params['filter_courses'] : 0;
            $replace = ' AND ' . $output[1] . ' = ' . $courseid;
            $sql = str_replace('%%BATCH_COURSE:' . $output[1] . '%%', $replace, $sql);
        }
