...
class report_assignmentparticipation extends reportbase implements report {
    ...
    function joins() {
        ...
        if(empty($this->params['filter_status']) || $this->params['filter_status'] == 'all') {
          $this->sql .= " LEFT JOIN {assign_submission} as asb ON asb.assignment = a.id AND asb.userid = ra.userid";
        } else if($this->params['filter_status'] == 'inprogress') {
          $this->sql .= "JOIN {assign_submission} as asb ON asb.assignment = a.id AND asb.userid = ra.userid AND asb.status = 'submitted'";
    ...
    function where() {
      ...
      $this->sql .=" WHERE c.visible = 1 AND cm.visible = 1 AND cm.deletioninprogress = 0 AND m.name = 'assign' AND m.visible = 1 ";
